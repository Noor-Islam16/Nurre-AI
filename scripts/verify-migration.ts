...
interface OnboardingResultData {
  user_id: string
  assessment_version: number
  adhd_presentation: string
...
async function verifyMigration() {
  ...
  const typedUsers = users as UserMigrationData[]
  ...
  const inconsistent = typedUsers.filter(u => 
    u.adhd_persona && !u.adhd_presentation && u.onboarding_version === 1
  )
  ...
  if (migrated.length > 0) {
    ...
    const mappingStats: Record<string, Record<string, number>> = {}
  ...
  if (resultsError) {
    ...
    if (results && results.length > 0) {
      const typedResults = results as OnboardingResultData[]
      ...
      const invalidThresholds = typedResults.filter(r => 
        r.inatt_endorsed > 6 || 
        r.hyper_endorsed > 6 || 
        r.total_endorsed > 12
      )

...
export async function POST(req: NextRequest) {
  try {
    ...
    const minuteBucket = rateLimit({
      key: `claim-code:regenerate:minute:${user.id}`,
      limit: 1,
      windowMs: MINUTE_MS,
    })
    ...
    const dailyBucket = rateLimit({
      key: `claim-code:regenerate:day:${user.id}`,
      limit: 5,
      windowMs: DAY_MS,
    })
    ...
    console.error("Failed to regenerate claim code", error)
    return NextResponse.json({ error: "Failed to regenerate claim code" }, { status: 500 })
  }
}
